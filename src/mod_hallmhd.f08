! =============================================================================
!> Computes Hall term contributions to the A-matrix and the boundary conditions.
module mod_hallmhd
...
contains
  ...
  !> Retrieves the normalised Hall factor m_i / q_i
  !! as described by Porth et al. (2014)
  ...
  subroutine get_hallterm(gauss_idx, eps, d_eps_dr, curr_weight, quadblock_Hall, &
    ...
    use mod_global_variables, only: hallfactor
    ...
    ! A(6, 6)
    factors(1) = mq * (eps_inv * k2 * B02 + k3 * B03) * ((k2 * eps_inv)**2 + k3**2) / rho0
    positions(1, :) = [6, 6]
    ...
    ! A(6, 7)
    factors(1) = -mq * k3 * eps_inv**2 * (k3 * drB02 - k2 * dB03) / rho0
    positions(1, :) = [6, 7]
    ! A(6, 8)
    factors(2) = mq * k2 * eps_inv**2 * (k3 * drB02 - k2 * dB03) / rho0
    positions(2, :) = [6, 8]
    ...
    call reset_positions(positions, 2)
    ! A(6, 7)
    factors(1) = -mq * k2 * eps_inv**2 * (k2 * B02 * eps_inv + k3 * B03) / rho0
    positions(1, :) = [6, 7]
    ! A(6, 8)
    factors(2) = -mq * k3 * (k2 * B02 * eps_inv + k3 * B03) / rho0
    positions(2, :) = [6, 8]
    ...
    ! A(7, 1)
    factors(1) = mq * k3 * eps_inv * (B02 * eps_inv * drB02 + B03 * dB03) / rho0**2
    positions(1, :) = [7, 1]
    ! A(7, 6)
    factors(2) = -mq * k3 * eps_inv * (k3 * drB02 - k2 * dB03) / rho0 &
                  - mq * k2 * k3 * eps_inv * (dB03 - B03 * drho0 / rho0) / rho0 &
                  + mq * k3**2 * (dB02 - B02 * drho0 / rho0 - d_eps_dr * eps_inv * B02) / rho0
    positions(2, :) = [7, 6]
    ! A(8, 1)
    factors(3) = -mq * k2 * eps_inv**2 * (B02 * eps_inv * drB02 + B03 * dB03) / rho0**2
    positions(3, :) = [8, 1]
    ! A(8, 6)
    factors(4) = mq * k2 * k3 * eps_inv**2 * drB02 / rho0 &
                  + mq * k3**2 * B03 * d_eps_dr * eps_inv / rho0 &
                  - mq * k2**2 * B02 * eps_inv**2 * drho0 / rho0**2 &
                  - mq * k2 * k3 * eps_inv * (dB02 - 2.0d0 * d_eps_dr * eps_inv * B02 &
                                                - B02 * drho0 / rho0) / rho0
    positions(4, :) = [8, 6]
    ...
    call reset_positions(positions, 2)
    ! A(7, 6)
    factors(1) = -mq * k2 * eps_inv * (k2 * eps_inv * B02 + k3 * B03) / rho0
    positions(1, :) = [7, 6]
    ! A(8, 6)
    factors(2) = -mq * k3 * (k2 * eps_inv * B02 + k3 * B03) / rho0
    positions(2, :) = [8, 6]
    ...
    call reset_positions(positions, 4)
    ! A(7, 7)
    factors(1) = mq * k3**2 * eps_inv * (k2 * eps_inv * B02 + k3 * B03) / rho0
    positions(1, :) = [7, 7]
    ! A(7, 8)
    factors(2) = - mq * k2 * k3 * eps_inv * (k2 * eps_inv * B02 + k3 * B03) / rho0
    positions(2, :) = [7, 8]
    ! A(8, 7)
    factors(3) = - mq * k2 * k3 * eps_inv**2 * (k2 * eps_inv * B02 + k3 * B03) / rho0 &
                  - mq * d_eps_dr * eps_inv**3 * k3 * drB02 / rho0
    positions(3, :) = [8, 7]
    ! A(8, 8)
    factors(4) = mq * (k2 * eps_inv)**2 * (k2 * eps_inv * B02 + k3 * B03) / rho0 &
                  + mq * d_eps_dr * eps_inv**3 * k2 * drB02 / rho0
    positions(4, :) = [8, 8]
    ...
    call reset_positions(positions, 4)
    ! A(7, 7)
    factors(1) = - mq * k3 * B03 * eps_inv * drho0 / rho0**2
    positions(1, :) = [7, 7]
    ! A(7, 8)
    factors(2) = mq * k3 * drB02 * eps_inv / rho0 - mq * k3 * dB02 / rho0 &
    ...
    factors(4) = - mq * k2 * eps_inv * (drB02 * eps_inv - dB02) / rho0 &
                  - mq * k3 * B03 * d_eps_dr * eps_inv / rho0 &
                  - 2.0d0 * mq * k2 * B02 * d_eps_dr * eps_inv**2 / rho0 &
                  - mq * k2 * B02 * eps_inv * drho0 / rho0**2
    positions(4, :) = [8, 8]
    ...
    call reset_positions(positions, 4)
    ! A(7, 7)
    factors(1) = - mq * k3 * dB03 * eps_inv / rho0
    positions(1, :) = [7, 7]
    ! A(7, 8)
    factors(2) = mq * k2 * dB03 * eps_inv / rho0
    positions(2, :) = [7, 8]
    ! A(8, 7)
    factors(3) = mq * k3 * drB02 * eps_inv**2 / rho0
    positions(3, :) = [8, 7]
    ! A(8, 8)
    factors(4) = - mq * k2 * drB02 * eps_inv**2 / rho0
    positions(4, :) = [8, 8]
    ...
    call reset_positions(positions, 2)
    ! A(7, 7)
    factors(1) = mq * eps_inv * (k2 * B02 * eps_inv + k3 * B03) / rho0
    positions(1, :) = [7, 7]
    ! A(8, 8)
    factors(2) = mq * (k2 * B02 * eps_inv + k3 * B03) / rho0
    positions(2, :) = [8, 8]
  ...
  subroutine hall_boundaries(quadblock_Hall, edge)
    use mod_global_variables, only: gauss_gridpts, dim_subblock, hallfactor
    ...
    ! Cubic * Quadratic
    allocate(positions(2, 2))
    allocate(surface_terms(2))

    ! surface term for element (7, 6)
    surface_terms(1) = mq * k2 * eps_inv * (k2 * eps_inv * B02 + k3 * B03) / rho0
    positions(1, :) = [7, 6]
    ! surface term for element (8, 6)
    surface_terms(2) = mq * k3 * (k2 * eps_inv * B02 + k3 * B03) / rho0
    positions(2, :) = [8, 6]
    ...
    surface_terms(1) = mq * k3 * eps_inv * dB03 / rho0
    positions(1, :) = [7, 7]
    ! surface term for element (7, 8)
    surface_terms(2) = -mq * k2 * dB03 * eps_inv / rho0
    positions(2, :) = [7, 8]
    ! surface term for element (8, 7)
    surface_terms(3) = -mq * k3 * drB02 * eps_inv**2 / rho0
    positions(3, :) = [8, 7]
    ! surface term for element (8, 8)
    surface_terms(4) = mq * k2 * eps_inv**2 * drB02 / rho0
    positions(4, :) = [8, 8]
    ...
    ! surface term for element (7, 7)
    surface_terms(1) = -mq * eps_inv * (k2 * B02 * eps_inv + k3 * B03) / rho0
    positions(1, :) = [7, 7]
    ! surface term for element (8, 8)
    surface_terms(2) = -mq * (k2 * B02 * eps_inv + k3 * B03) / rho0
    positions(2, :) = [8, 8]
