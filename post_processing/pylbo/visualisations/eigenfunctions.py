...
class EigenfunctionHandler:
    ...
    def connect_figure_events(self, spectrum, ef_fig, ef_ax):
        ...
        def on_left_click(event):
            ...
            if event.button == 1:
                ...
                index = self._find_spectrum_point_index(
                    spectrum, event.xdata, event.ydata
                )
                if index is None:
                    return
                ...
                if index not in self._ev_indices:
                    ...
                    spectrum.ax.plot(
                        self.ds.eigenvalues[index].real,
                        self.ds.eigenvalues[index].imag,
                        "rx",
                        markersize=8,
                        picker=10,
                        label="eigenvalue",
        ...
        def on_right_click(event):
            ...
            if event.mouseevent.button == 3:
                if (
                    hasattr(event.artist, "get_label")
                    and event.artist.get_label() == "eigenvalue"
                ):
                    # right clicking returns (x, y) as ([x], [y]) so use unpacking
                    index = self._find_spectrum_point_index(
                        spectrum, *event.artist.get_xdata(), *event.artist.get_ydata()
                    )
                    if index is None:
                        return
        ...
        def on_key_press(event):
            ...
            if event.key == "d":
                ...
                for artist in spectrum.ax.get_children():
                    if (
                        hasattr(artist, "get_label")
                        and artist.get_label() == "eigenvalue"
                    ):
                        artist.remove()
                self._ef_ax.clear()
            ...
            if event.key == "w":
                ...
                pylboLogger.info(
                    f"selected indices: {[idx for idx in self._ev_indices]}"
                )
    ...
    def _find_spectrum_point_index(self, spectrum, x, y):
        ...
        ev_x_pixels, ev_y_pixels = spectrum.ax.transData.transform(
            (ev_found.real, ev_found.imag)
        )
        ...
        pixel_criterion = 15
        dist_pixels = np.sqrt(
            (ev_x_pixels - click_x_pixels) ** 2 + (ev_y_pixels - click_y_pixels) ** 2
        )
    ...
    def _plot_eigenfunctions(self):
        ...
        for ev_idx, efs in zip(self._ev_indices, self.ef_container):
            ...
            if (
                ...
                and self.ds.geometry == "cylindrical"
            ):
                ef = ef * self.ef_grid
