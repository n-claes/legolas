...
class LEGOLASDataContainer:
    ...
    def read_omegas(self):
        ...
        return read_stream_data(fname, content_type=np.complex, rows=1, cols=self.mat_gridpts)[0]


    def read_matA(self):
        fname = (self.src_dir / self.fname_matA).resolve()
        return read_stream_data(fname, content_type=np.complex, rows=self.mat_gridpts, cols=self.mat_gridpts)


    def read_matB(self):
        fname = (self.src_dir / self.fname_matB).resolve()
        return read_stream_data(fname, content_type=np.float64, rows=self.mat_gridpts, cols=self.mat_gridpts)


    def read_grid(self):
        fname = (self.src_dir / self.fname_grid).resolve()
        return read_stream_data(fname, content_type=np.float64, rows=1, cols=self.ef_gridpts)[0]
