...
class SingleSpectrum:
    ...
    def __init__(self, ds, fig=None, ax=None):
        if isinstance(ds, list):
            raise TypeError(
                "SingleSpectrum needs a single LegolasDatacontainer instance, "
                "not a list."
            )
        self.ds = ds
    ...
    def plot_spectrum(self, annotate_continua=True):
        ...
        self.annotate_continua = annotate_continua
        self.ax.plot(
            self.ds.eigenvalues.real, self.ds.eigenvalues.imag, ".b", alpha=0.8
        )
    ...
    def _annotate_continua(self, fig, ax):
        ...
        draw_region(
            self.ds.continua["wth"],
            facecolor="green",
            legend_lbl=r"$\Omega_{th}$ thermal continuum",
            draw_horizontal=True,
        )
        draw_region(
            self.ds.continua["dopp"],
            facecolor="grey",
            legend_lbl=r"$\Omega_0$ Doppler shift",
        )
        legend = ax.legend(loc="best")

        # retrieve legend patches and items for interactive enabling
        # of continuum regions
        regions = {}
    ...
    def plot_equilibria(self, fig=None, ax=None):
        ...
        for idx, var in enumerate(self.ds.header.get("equil_names")):
            ...
            if (values == 0).all():
                continue
            ax.plot(
                self.ds.grid_gauss, values, "-o", markersize=2, label=var, alpha=0.8
            )
        ax.set_xlabel("grid")
...
class MultiSpectrum:
    ...
    def _check_datasets(self, equilibrium):
        ...
        for ds in self.datasets:
            if not equilibrium == ds.header.get("eq_type"):
                raise InconsistentMultirunFile(
                    ds.datfile, expected=equilibrium, found=ds.header.get("eq_type")
                )
    ...
    def plot_precoded_run(self, annotate_continua=False):
        ...
        if equilibrium == "gravito_acoustic":
            ...
            raise ValueError(
                "Automatically plotting the spectrum failed, should be done manually."
            )
        if annotate_continua:
    ...
    def _annotate_continua(self):
        ...
        self.ax.fill_between(
            xvals, slowmin, slowmax, color="red", alpha=0.3, label="slow_continuum"
        )
        ...
        self.ax.fill_between(
            xvals, alfvmin, alfvmax, color="cyan", alpha=0.3, label="Alfven continuum"
        )
    ...
    def _plot_flux_tube(self):
        ...
        self.ax.axhline(
            cse / cs, label="$c_{se}$", lw=1, color="red", linestyle="dotted", alpha=0.6
        )
