...
def test_params(ds_test, ds_answer):
    ...
    assert params.pop("g") == pytest.approx(100.0) == answ_params.pop("g")
    assert params.pop("alpha") == pytest.approx(-np.pi) == answ_params.pop("alpha")
    assert params.pop("theta") == pytest.approx(0.0) == answ_params.pop("theta")
    assert params.pop("p1") == pytest.approx(1.0) == answ_params.pop("p1")
    assert params.pop("p2") == pytest.approx(2.0) == answ_params.pop("p2")
    assert params.pop("p3") == pytest.approx(1.0) == answ_params.pop("p3")
    assert params.pop("p4") == pytest.approx(0.5 * np.pi) == answ_params.pop("p4")
    assert params.pop("tau") == pytest.approx(4.0) == answ_params.pop("tau")
    assert len(params) == 0
    assert len(answ_params) == 0


def test_eq_type(ds_test):
    assert ds_test.eq_type == "RTI_KHI"
