...
contains
  ...
  module procedure solve_arpack_general
    ...
    call log_message( &
      "   Total number of OP*x operations: " // str(arpack_cfg%iparam(9)), &
      level="info", &
      use_prefix=.false. &
    )
    call log_message( &
      "   Total number of B*x operations: " // str(arpack_cfg%iparam(10)), &
      level="info", &
      use_prefix=.false. &
    )
    call log_message( &
      "   Total number of re-orthogonalization steps: " // str(arpack_cfg%iparam(11)), &
      level="info", &
      use_prefix=.false. &
    )
