...
2. **MPI-AMRVAC setup.** From the same SymPy-based equilibrium definition, with the appropriate accompanying code configuration details, GIMLI can also generate a user module `mod_usr.t` and parfile for MPI-AMRVAC. The perturbation of this equilibrium is provided by selecting eigenmodes from a Legolas spectrum to construct a superposition of linear solutions with a specified amplitude. This linear solution is then interpolated to the grid in MPI-AMRVAC.
