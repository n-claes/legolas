...
```
which will place the user module in the current directory and create a subdirectory for the parfile. The location for each file can be specified with the optional argument `loc`. For the parfile, the creation of a subdirectory can be suppressed by passing `subdir=False`. The last step is to write the Legolas data in a format that MPI-AMRVAC can read,
```python
amrvac_setup.prepare_legolas_data()
```
Again, `loc` is used to set the target directory.
