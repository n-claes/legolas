...
contains
  ...
  module subroutine tc_pinch_eq()
    ...
    real(dp)    :: r, h, Rrat, A, B, A2, B2, K1, K2, K3, Tstart, Tend, Ta, Ha, Re, Pm
    integer     :: i
    ...
    Tend = ( 0.5d0 * K1 * x_end**2 &
              + 2.0d0 * K2 * log(x_end) - 0.5d0 * K3 / x_end**2 &
    ...
    do i = 1, gauss_gridpts
      ...
      if ((Tstart > 0) .and. (Tend > 0)) then
        ...
        T_field % T0(i) = 2.0d0 * max(abs(Tstart), abs(Tend)) + ( 0.5d0 * K1 * r**2 &
                          + 2.0d0 * K2 * log(r) - 0.5d0 * K3 / r**2 &
                          - 0.5d0 * (A2 * r + B2 / r)**2 ) / cte_rho0
      end if
