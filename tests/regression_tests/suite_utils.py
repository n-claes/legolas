...
def compare_eigenvalues(values_test, values_answer, ds_name):
    ...
    if fail_idxs:
        for i in fail_idxs:
            ...
            print('FAIL: {} >> {} (test >> answer)'.format(val_test, val_answ))
        ...
        print(">>> FAILED EIGENVALUES: {}/{}".format(len(fail_idxs), len(values_answer)))
...
def compare_eigenfunctions(values_test, values_answer, use_abs=True):
    if use_abs:
        vals_test_real, vals_test_imag = np.abs(values_test.real), np.abs(values_test.imag)
        vals_answ_real, vals_answ_imag = np.abs(values_answer.real), np.abs(values_answer.imag)
    else:
    ...
    for val_test_arr, val_answ_arr in zip([vals_test_real, vals_test_imag], [vals_answ_real, vals_answ_imag]):
